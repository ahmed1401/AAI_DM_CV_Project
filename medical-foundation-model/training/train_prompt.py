"""
Entraînement Prompt Learning (Visual Prompts, backbone gelé) - 15 epochs
"""
import torch
...
def main(CHEST_DIR: str, BRATS_DIR: str, HAM_DIR: str, device: str = None):
    ...
    model = MultiTaskViT(vit, freeze_backbone=True, use_adapters=False, use_prompts=True, prompt_length=10, img_size=IMG_SIZE).to(device)

    history, results = train_model(model, (chest_train_loader, brats_train_loader, ham_train_loader), (chest_test_loader, brats_test_loader, ham_test_loader), 'Visual Prompt Tuning', epochs=EPOCHS, lr=1e-4, device=device)
    print("\n📊 Résultats Prompt:", results)

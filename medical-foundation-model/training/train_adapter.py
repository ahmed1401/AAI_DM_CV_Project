"""
Entraînement Adapter Layers (backbone gelé, adapters actifs) - 15 epochs
"""
import torch
...
def main(CHEST_DIR: str, BRATS_DIR: str, HAM_DIR: str, device: str = None):
    ...
    model = MultiTaskViT(vit, freeze_backbone=True, use_adapters=True, use_prompts=False, adapter_hidden=64, img_size=IMG_SIZE).to(device)

    history, results = train_model(model, (chest_train_loader, brats_train_loader, ham_train_loader), (chest_test_loader, brats_test_loader, ham_test_loader), 'Adapter Layers', epochs=EPOCHS, lr=1e-4, device=device)
    print("\n📊 Résultats Adapters:", results)
